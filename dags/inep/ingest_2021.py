...
def ingest():
    path = '/data/inep/2021'
    df_igc = pd.read_excel(f'{path}/IGC_2021.xlsx', skipfooter=2)
    ...
    df_cpc = pd.read_excel(f'{path}/resultados_cpc_2021.xlsx', skipfooter=4)
    ...
    df_enade = pd.read_excel(f'{path}/conceito_enade_2021.xlsx', skipfooter=3)
    ...
    df_idd = pd.read_excel(f'{path}/IDD_2021.xlsx', skipfooter=3)
    df_idd = df_idd[[' Código da IES',' Código do Curso', ' IDD (Contínuo)',' IDD (Faixa)']]
    df_idd.columns = ['codigo_ies','codigo_curso','idd_continuo','idd_faixa']
    df_idd.replace(to_replace='SC', value=np.NaN, inplace=True)
    ...
    log.info(result)
    result.to_parquet('/data/output/inep_2021.parquet')